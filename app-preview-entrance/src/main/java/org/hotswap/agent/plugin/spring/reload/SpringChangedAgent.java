...
public class SpringChangedAgent implements SpringListener<SpringEvent<?>>, Comparable<SpringChangedAgent> {
    ...
    public static void reload(long changeTimeStamps) {
        ...
        try {
            // 移除Mybatis多余的SpringChangedAgent
            removeDuplicateMybatisSpringChanged();
    ...
    private static void removeDuplicateMybatisSpringChanged() {
        LinkedList<DefaultListableBeanFactory> duplicateBeanFactories = new LinkedList<>();

        for (DefaultListableBeanFactory beanFactory : springChangeAgents.keySet()) {
            String[] beanDefinitionNames = beanFactory.getBeanDefinitionNames();
            for (String beanDefinitionName : beanDefinitionNames) {
                if (beanDefinitionName.contains("MapperScannerRegistrar") && beanDefinitionNames.length == 1) {
                    duplicateBeanFactories.add(beanFactory);
                    break;
                }
            }
        }

        for (DefaultListableBeanFactory duplicateBeanFactory : duplicateBeanFactories) {
            springChangeAgents.remove(duplicateBeanFactory);
        }
    }
